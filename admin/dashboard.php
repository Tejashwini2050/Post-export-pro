...
<section class="dashboard">
   ...
   <div class="box-container">
      ...
      <div class="box">
         <?php
            $total_pendings = 0;
            $select_pendings = $conn->prepare("SELECT * FROM `orders` WHERE payment_status = ? AND `pid` IN(SELECT `pid` from `products` WHERE `adminid` =?)");
      ...
      <div class="box">
         <?php
            $total_completes = 0;
            $select_completes = $conn->prepare("SELECT * FROM `orders` WHERE payment_status = ? AND `pid` IN(SELECT `pid` from `products` WHERE `adminid` =?)");
      ...
      <div class="box">
         <?php
            $select_orders = $conn->prepare("SELECT * FROM `orders` WHERE `pid` IN(SELECT `pid` from `products` WHERE `adminid` =?)");
      ...
      <div class="box">
         <?php
            $select_users = $conn->prepare("SELECT * FROM `users` WHERE id IN (SELECT `user_id` FROM `orders` WHERE `pid` IN(SELECT `pid` from `products` WHERE `adminid` =?))");
