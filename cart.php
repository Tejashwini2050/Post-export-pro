...
<section class="products shopping-cart">
   ...
   <?php
      // Fetch the first PID from the cart (you can adjust this logic)
      

      $select_cart = $conn->prepare("SELECT pid, name, price, quantity FROM `cart` WHERE user_id = ?");
      $select_cart->execute([$user_id]);

      if ($select_cart->rowCount() > 0) {
         $fetch_cart = $select_cart->fetch(PDO::FETCH_ASSOC);
         // Get the PID from the first cart item
         $pid = $fetch_cart['pid'];
         ...
         // Your existing code to display cart items
         // ...
      } else {
         echo '<p class="empty">your cart is empty</p>';
      }
   ...
   <div class="cart-total">
      ...
      <a href="checkout.php?pid=<?= $pid; ?>&pname=<?= $pname; ?>&quantity=<?= $quantity; ?>&price=<?= $price; ?>" class="btn <?= ($grand_total > 1) ? '' : 'disabled'; ?>">proceed to checkout</a>
